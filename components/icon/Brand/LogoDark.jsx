function LogoDark({ width = '401', height = '400' }) {
  return (
    <svg xmlns='http://www.w3.org/2000/svg' width={width} height={height} fill='none' viewBox='0 0 400 400'>
      <rect width='400' height='400' fill='#101010' rx='100'></rect>
      <g fill='#80F906' fillRule='evenodd' clipPath='url(#clip0_1_1140)' clipRule='evenodd'>
        <path d='M75 349.999c88.77-7.754 136.688-49.934 212.661-183.621 0 0-100.486 187.4-201.729 173.846L75 349.999z'></path>
        <path d='M324.724 47.603c-24.998 61.938-74.388 107.191-111.334 140.831-18.474 16.82-34.729 32.98-44.172 52.646-9.443 19.667-11.684 43.669-2.897 73.579l6.523 1.202s-.692-3.869-.85-4.426c9.118 1.505 44.241 4.288 78.921-23.114 40.197-31.759 78.491-99.129 73.671-240.723l.139.004h-.001zm-6.01 17.157c1.251 128.808-34.984 190.176-71.792 219.261-35.393 27.964-69.932 26.615-75.359 26.229-7.8-27.921-5.417-48.824 3.261-66.894 8.89-18.515 24.517-34.193 42.868-50.901 33.857-30.825 76.795-65.194 101.028-127.691l-.006-.004z'></path>
      </g>
      <defs>
        <clipPath id='clip0_1_1140'>
          <path fill='#FDFDFD' d='M0 0H250V300H0z' transform='translate(75 50)'></path>
        </clipPath>
      </defs>
    </svg>
  )
}

export default LogoDark
