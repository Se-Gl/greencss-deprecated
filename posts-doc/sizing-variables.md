...
| ----------- | -------------- | ------------------ |
| sm:         | 0px - 480px    | .max-h-fit-content |
| md:         | 480px - 768px  | .max-h-fit-content |
| lg:         | 768px - 1080px | .max-h-fit-content |

omenCSS media queries can also be combined with active, focus and hover states. For example, use `sm:max-h-fit-content` to apply the `max-h-fit-content` utility at only small screen sizes at or below 480px. Or `active:md:max-h-fit-content` to apply the `max-h-fit-content` utility at only between small screen sizes (480px) and medium screen sizes (768px) and on active state.

1. The structure is as follows: `{conditionally state}:{media query}:{classname}` p.e. `active:md:m-auto`
